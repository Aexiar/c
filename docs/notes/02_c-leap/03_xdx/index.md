...
> [!NOTE]
>
> * ① `()` 和 `[]` 的优先级相同，结合方向是从`左`到`右`，如：`int (*p)[6]`中的 `()` 和 `[]`的优先级虽然相同，但是由于结合方向是从`左`到`右`，编译器会先解析 `(*p)`，表明 `p` 是一个`指针变量`；再解析 `int[6]`，表示 `p` 是一个指向包含 `6` 个 `int` 类型数组的`指针变量`。
> * ② `*` 的优先级要比 `()` 和 `[]` 低，如：`int *p[6]` 中的 `[]`优先级高于 `*`，相当于 `int *(p[6])`，编译器会先解析 `p[6]`，表明 `p` 是包含 `6` 的元素的`数组`；再解析 `int*`，表示 `p` 数组中的元素全是 `int *` 类型。 
...
### 2.1.4 CLion 中如何查看`数组名`或 `&数组名`的数据类型
