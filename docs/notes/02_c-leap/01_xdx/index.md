...
* 我们知道，CPU 和内存也是有速度差异的，在 CPU 内部集成了 L1、L2 和 L3 级别的高速缓存；并且，通过这种多级缓存机制，CPU 可以快速从缓存中获取指令和数据，而无需频繁访问较慢的主内存（RAM），这减少了内存带宽瓶颈对 CPU 性能的限制。
* 但是，如果 CPU 中的高速缓存 L1、L2 和 L3 中如果没有对应的指令和数据，就需要通过 CPU 内部的 MMU 部件中的 TLB（Translation Lookaside Buffer，后备缓冲区） 来缓存最近使用的虚拟地址到物理地址的映射，从而避免每次都去查页表，提升地址转换速度。
