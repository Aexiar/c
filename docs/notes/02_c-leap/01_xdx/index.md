...
* 之前，也提及过，对于 `32` 位的操作系统而言，因为其地址总线宽度为 `32` 位，所以内存地址是从 `0x00000000` 到 `0xFFFFFFFF`，并且内存地址是从小到大依次排列的，如下所示：

![](./assets/9.svg)

* 而对于 `64` 位的操作系统而言，因为其地址总线宽度为 `64` 位，内存地址是从 `0x0000000000000000` 到 `0xFFFFFFFFFFFFFFFF`，并且内存地址是从小到大依次排列的，如下所示：
...
```c
// (int *)0x1234 中的 (int *) 是强转，就是告诉编译器，0x1234 是一个地址
