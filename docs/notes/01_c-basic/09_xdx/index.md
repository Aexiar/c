...
> [!NOTE]
>
> * ① 表达式可以非常简单，如：一个单独的常量、变量。
> * ② 表达式也可以非常复杂，如：包含多个运算符或函数调用的结合。
...
>
> * ① 流程控制语句，后面再讲解！！！
> * ② 其实，区分`语句`和`表达式`最明显的领域应该属于前端 JavaScript 框架中的 React，其明确要求 JSX 是表达式而不能是语句，如下所示：
>
> ```c
...
* 在`表达式`中，最重要、最核心的就是连接表达式中常量和变量的运算符了。
...
> [!NOTE]
>
> 在 C23 标准中，`printf` 和 `scanf` 都支持二进制整数，其对应的格式占位符是 `%b`。
